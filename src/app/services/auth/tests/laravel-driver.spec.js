import Driver from '../drivers/laravel';

function getItem(name) {
  return JSON.parse(window.localStorage.getItem(name));
}

describe('LaravelDriver', () => {
  beforeEach(() => {
  ...
  it('should login through a Laravel API', () => {
    const response = {
      data: {
  ...
  it('should set and cache auth data', () => {
    ...
    expect(getItem('auth.laravel.accessToken')).to.equal('accessToken');
    expect(getItem('auth.laravel.expiresAt') >= time).to.be.true;
  });
  it('should set and cache auth data', () => {
    const http = {};
    const config = {};
    ...
    expect(getItem('auth.laravel.accessToken')).to.be.null;
    expect(getItem('auth.laravel.expiresAt')).to.be.null;
  });
